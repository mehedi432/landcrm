...
def reverse_geocode(lat, lon, api_key):
    ...
    if response.status_code == 200:
        ...
        frappe.logger().info(f"[Google API Response] {json.dumps(geo_data, indent=2)}")
        print("Raw Google API response:", json.dumps(geo_data, indent=2))

        if geo_data.get("results"):
            return geo_data["results"][0]
        else:
            frappe.msgprint("No results returned from Google Maps API.")
            print("No results from Google.")
    else:
        frappe.msgprint(f"Failed to contact Google Maps API: {response.status_code}")
...
class TractProfile(Document):
    ...
    def extract_and_save_tract_data(self):
        ...
        # ✅ Call reverse geocoding to update child rows
        self.update_tracts_with_parent_location(google_maps_api_key)

        # Return full updated child table rows as list of dicts
        return [row.as_dict() for row in self.tracts]

    def update_tracts_with_parent_location(self, google_maps_api_key):
        lat = self.latitude
        lon = self.longitude

        print(f"Using lat: {lat}, lon: {lon}")
...
@frappe.whitelist()
def extract_tract_data_for_doc(docname):
    doc = frappe.get_doc("Tract Profile", docname)
    return doc.extract_and_save_tract_data()
